...
TOOLS:
1. vault_search(tags=[], exclude_tags=[], min_playtime=0, max_playtime=0, hltb_max=0, status=[])
   - Use this to find games in the database.
...
3. get_game_details(game_name)
   - Use this to get the description, price, and deep details of a SPECIFIC game found in the search.

3. get_reviews(game_name)
   - Use this to get steam reviews for a specific game. Use to dive into one specific game quality, not for a batch of games.
...
RULES:
1. DO NOT LOOP. If a search returns games, STOP searching and present them to the user.
            ...
            try:
                if tool_name == "vault_search":
                    ...
                    if count == 0:
                        ...
                        system_hint = f"System Note: Search returned {count} games. Use these results for the user."

                        # Slice results to save tokens
                        lean_results = [
                            {k: v for k, v in res.items() if
                             k in ['name', 'playtime_forever', 'calculated_status', 'hltb_main']}
                    ...
                    tool_output_str = br.get_reviews_byname(params.get('game_name'), 10)

            except Exception as e:
                tool_output_str = f"Error: {str(e)}"

            # --- APPEND HISTORY WITH HINTS ---
